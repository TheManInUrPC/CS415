...
public class ARegularPoly implements AShape
{
    ...
    private void makePolygon( int locX, int locY )
    {
        ...
        int y[] = new int[ _x.length ];
       
        for( int i = 0; i < _x.length; i++ )
        {
            x[i] = ( int ) Math.round( _x[i] + locX - _ulX );
        }
        
        for( int i = 0; i < _y.length; i++ )
        {
            y[i] = ( int ) Math.round( _y[i] + locY - _ulY );
        }
    ...
    public void draw( java.awt.Graphics2D aBrush ) 
    {
        ...
        aBrush.draw( _polygon ); // "this" class, AEllipse, is subclass
    ...
    public void setSize( int aWidth, int aHeight ) 
    {
        ...
        {
            System.out.println( "ERROR aWidth and aHeight are not equal" );
            int avrge = ( aWidth + aHeight / 2 );
            _radius = avrge;
            setRadius( _radius / 2 );
        }
